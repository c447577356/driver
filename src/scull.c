...
nt scull_major = SCULL_MAJOR;  
nt scull_minor = 0;  
nt scull_nr_devs = SCULL_NR_DEVS;  
nt scull_quantum = SCULL_QUANTUM;  
nt scull_qset = SCULL_QSET;  
 
odule_param(scull_major,int,S_IRUGO);  
odule_param(scull_minor,int,S_IRUGO);  
odule_param(scull_nr_devs,int,S_IRUGO);  
 
ODULE_LICENSE("Dual BSD/GPL");  
 
truct scull_dev *scull_devices;  
...
* scull_trim 负责释放整个数据区，并且在文件以写入放送打开时由scull_open调用，它简单遍历链表，并释放所有找到的 
  量子和量子集 */  
nt scull_trim(struct scull_dev *dev)  
...
nt scull_open(struct inode *inode,struct file *filep)  
...
nt scull_release(struct inode *inode, struct file *filep)  
  
   return 0;  
  
 
truct scull_qset *scull_follow(struct scull_dev *dev,int n)  
